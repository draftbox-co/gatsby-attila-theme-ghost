(async () => {
  ...
  if (gatsbyConfig.plugins.length) {
    const index = gatsbyConfig.plugins.findIndex(obj => obj.resolve === '@draftbox-co/gatsby-theme-ghost-attila');
    console.log(index);
    if (index > -1) {
      ...
      if (pluginOptions.options.ghostConfig) {
        ...
        if (environment === 'development') {
          config = Object.assign(config, pluginOptions.options.ghostConfig.development);
        }
        if (environment === 'production') {
          config = Object.assign(config, pluginOptions.options.ghostConfig.production);
        }
        try {
          ...
          if (resp.status === 200) {
            ...
            console.log(path.resolve(__dirname, '..', `gatsby-theme-ghost-attila`, 'static'));
            if (siteData.title) {
              ...
              newSiteConfig.metadata.title = siteData.meta_title || siteData.title || '';
              newSiteConfig.metadata.description = siteData.meta_description || siteData.description || '';
              newSiteConfig.twitterCard.title = siteData.twitter_title || siteData.title || '';
              newSiteConfig.twitterCard.description = siteData.twitter_description || siteData.description || '';
              newSiteConfig.twitterCard.imageUrl = siteData.twitter_image || siteData.cover_image || '';
              newSiteConfig.facebookCard.title = siteData.og_title || siteData.title ||  '';
              newSiteConfig.facebookCard.description = siteData.og_description || siteData.description || '';
              newSiteConfig.facebookCard.imageUrl = siteData.og_image || siteData.cover_image || '';
              ...
              if (siteData.logo && siteData.logo !== '') {
                ...
                if (resp.status == 200) {
                  ...
                  fs.writeFileSync(path.resolve(__dirname, '..', `gatsby-theme-ghost-attila`, 'static', fileName), bufferData);
              ...
              if (siteData.cover_image && siteData.cover_image !== '') {
                ...
                if (resp.status == 200) {
                  const bufferData = await resp.buffer();
                  const fileName = `coverImage${path.extname(siteData.cover_image)}`;
                  fs.writeFileSync(path.resolve(__dirname, '..', `gatsby-theme-ghost-attila`, 'static', fileName), bufferData);
              ...
              if (siteData.twitter_image && siteData.twitter_image !== '') {
                ...
                if (resp.status == 200) {
                  const bufferData = await resp.buffer();
                  const fileName = `twitterCardImage${path.extname(siteData.twitter_image)}`;
                  fs.writeFileSync(path.resolve(__dirname, '..', `gatsby-theme-ghost-attila`, 'static', fileName), bufferData);
              ...
              if (siteData.og_image && siteData.og_image !== '') {
                ...
                if (resp.status == 200) {
                  const bufferData = await resp.buffer();
                  const fileName = `fbCardImage${path.extname(siteData.og_image)}`;
                  fs.writeFileSync(path.resolve(__dirname, '..', `gatsby-theme-ghost-attila`, 'static', fileName), bufferData);
              ...
              newSiteConfig.identity.logoUrl = imageJSON[newSiteConfig.identity.logoUrl];
              newSiteConfig.identity.coverUrl = imageJSON[newSiteConfig.identity.coverUrl];
              newSiteConfig.identity.alternateLogoUrl = imageJSON[newSiteConfig.identity.alternateLogoUrl];
              newSiteConfig.twitterCard.imageUrl = imageJSON[newSiteConfig.twitterCard.imageUrl];
              newSiteConfig.facebookCard.imageUrl = imageJSON[newSiteConfig.facebookCard.imageUrl];
              const finalConfig = Object.assign({}, pluginOptions.options.siteConfig, siteConfig, {
              ...
              fs.writeFileSync(path.join(__dirname, 'gatsby-config.js'), `module.exports = ${JSON.stringify(gatsbyConfig, null, 2)}`);
              console.log('done');
            }
          } else {
